...
def kLDiv(true_posterior,estimated_posterior):
    ...
    for x in samples[1:]:
        divergence = np.hstack([divergence,0.5*entropy(x[:,0],x[:,1])+0.5*entropy(x[:,1],x[:,0])])
    return divergence
...
#################################### Arch Model #######################################################################################
#ARCH true posterior calculation:
class ARCH_TP:
...
#Arch Model
class ArchModel:
    def __init__(self,theta1_minRange,theta1_maxRange,theta2_minRange,theta2_maxRange,randomSeed=432,T=100,skip=1):
    ...
    def GeneratorThetaGiven(self,theta_values, n_samples,e=None,epsilon=None):
        ...
        returned_y = e.reshape((n_samples,1))
        ...
        for i in range(1,T):
            ...
            if(i%skip==0):
                returned_y=np.hstack([returned_y, y_t])
            
        #preprocessing
        returned_y = returned_y/np.sqrt(T/skip)
    ...
    def klDivergence(self,mu_values,observed_data,estimated_posterior):
        obj = ARCH_TP()
        true_posterior = np.apply_along_axis(lambda obsData: obj.truePosterior(obsData,mu_values), 1, observed_data)
        return kLDiv(true_posterior,estimated_posterior)
