...
kRBF1 = lambda X1,X2: kernelRBF(X1,X2,gamma1)
clf1=kC.KernelClassifier(kRBF1)

with open('Standardized_RBF_parametersAndRegScales.pickle', 'rb') as f:
     parametersAndRegScales = pickle.load(f)
[(gamma2,regScale2)] = parametersAndRegScales

kRBF2 = lambda X1,X2: kernelRBF(X1,X2,gamma2)
clf2=kC.KernelClassifier(kRBF2, withStandardization =True)

with open('Fourier_parametersAndRegScales.pickle', 'rb') as f:
     parametersAndRegScales = pickle.load(f)
[(gamma3,regScale3)] = parametersAndRegScales

kF1 = lambda X1,X2: kernelFourier(X1,X2,gamma3)
clf3=kC.KernelClassifier(kF1)

with open('Standardized_Fourier_parametersAndRegScales.pickle', 'rb') as f:
     parametersAndRegScales = pickle.load(f)
[(gamma4,regScale4)] = parametersAndRegScales

kF2 = lambda X1,X2: kernelFourier(X1,X2,gamma4)
clf4=kC.KernelClassifier(kF2,withStandardization =True)
